...
mini_diameter = 44.6*2;       // Disc diameter
base_thickness = 4;       // Base disc thickness (layers before color swap)
raised_height = 0.4;        // Height of raised features (after color swap)
...
module decorative_rings() {
    ...
    difference() {
        ...
        circle(d=ring_diameter - ring_width);
    }

    // Inner ring (inside the text) - scaled for 75mm disc
    difference() {
        circle(d=ring_diameter-11);
        circle(d=ring_diameter-11 - ring_width);
    }
}
...
module circular_text() {
    ...
    text_string = " #181840 ";  // Space at end for separation
    radius = 38;    // Distance from center (scaled for 75mm disc)
    text_size = 3;
